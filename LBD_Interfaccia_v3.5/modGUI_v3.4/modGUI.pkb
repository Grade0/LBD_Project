...
begin
  ...
  modGUI.ApriDiv(classe => 'bgimg');
    ...
    modGUI.ApriDiv(ident => 'b1' ,classe => 'form-box');
        ...
        htp.prn(' <div class="form-g">
                  <input type="password" class="password form-control" required name="v_Pwd" placeholder="Inserire password...">
                  </div>');
  ...
  modGUI.ChiudiDiv;
    ...
    in questo caso non deve comparire il menu completo */
    modGUI.ChiudiDiv;
...
lunghezzaMax varchar2 default '', require boolean default false) is
begin
  if(center)then
    htp.print('
    <div class="form-group row">
      ...
      <div class="col-lg-push-2 col-lg-3">');
        ...
        if(require) then
          htp.print(' <textarea class="form-control" required rows="3" name="'||nome||'" maxlength="'||lunghezzaMax||'">' || valore||'</textarea>
        </div>
      </div>');

        else
          htp.print(' <textarea class="form-control" rows="3" name="'||nome||'" maxlength="'||lunghezzaMax||'">' || valore||'</textarea>
        </div>
      </div>');
        end if;
      ...
      htp.print('
        <div class="form-group row">
          <label class="col-lg-2 control-label">' || etichetta ||'</label><div class="col-lg-3">');
      if(require) then
